...
def process_and_visualize_peaks(signal, t, window_size, peak_window_size, threshold_factor):
    ...
    trace_index = 0  # Índice para numerar los archivos JSON
    i = 0
    while i < len(signal) - window_size + 1:
        ...
        # Encontrar los picos en la ventana
        peaks, _ = find_peaks(peak_window)
        if len(peaks) < 2:
            i += step_size
            continue

        # Obtener los dos picos más altos
        sorted_peaks = sorted(
            peaks, key=lambda x: peak_window[x], reverse=True)[:2]
        highest_peak_index = sorted_peaks[0]
        second_highest_peak_index = sorted_peaks[1]
        ...
        # Comparar la altura de los dos picos
        peak_difference = peak_window[highest_peak_index] - \
            peak_window[second_highest_peak_index]
        ...
        if len(post_peak_signal) > 0 and np.any(np.isclose(post_peak_signal, peak_window[init_index], atol=0.1)):
            counter += 1
            # Guardar la traza en un archivo JSON junto con su velocidad
            data = {
                "numero_de_picos": len(peaks),
                "velocidad_calculada": velocity_kmh,
                "velocidad_real": 0,  # Este campo debe ser llenado con la velocidad real si se tiene
                "senal_en_bruto": peak_window.tolist(),
                "senal_suavizada": smoothed_signal.tolist(),
                "senal_derivada": smoothed_derivative.tolist(),
                "pico_mas_alto": peak_window[highest_peak_index],
                "segundo_pico_mas_alto": peak_window[second_highest_peak_index],
            ...
            ax1.plot(second_highest_peak_time,
                     peak_window[second_highest_peak_index], 'yo', label='Segundo pico más alto')
